(function (root, factory) {
  ...
  function isVersionBasePath(pathname) {
    ...
    return /\/(?:api\/)?v\d+(?:(?:beta|alpha)\d*)?$/i.test(value);
  }

  function isExactRequestEndpoint(input) {
    const normalized = normalizeDiscoveryURL(input);
    if (inferRequestFormat(normalized)) return true;
    const url = new URL(normalized);
    return !isVersionBasePath(url.pathname);
  }
  ...
  function buildEndpointDiscoveryPlan(input, options = {}) {
    ...
    const exact = isExactRequestEndpoint(baseUrl);
    if (exact) {
      return {
        baseUrl,
        exact: true,
        ...
        models: [],
        requests: [{
          id: format || 'generic-get',
          url: baseUrl,
          method: format ? 'POST' : 'GET',
          format: format || 'none',
