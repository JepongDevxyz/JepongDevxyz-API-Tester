...
function buildBody(format, input) {
  ...
  if (format === 'openai_chat') {
    return JSON.stringify({ model, messages: [{ role: 'user', content: prompt }], max_tokens: maxTokens, temperature: 0 });
  }
  if (format === 'openai_responses') {
    return JSON.stringify({ model, input: prompt, max_output_tokens: maxTokens });
  }
  ...
  if (format === 'cohere_chat') {
    return JSON.stringify({ model, messages: [{ role: 'user', content: prompt }], max_tokens: maxTokens });
  }
...
function joinUrl(baseUrl, path) {
  ...
  if (url.origin !== base.origin) throw new Error('Endpoint path must stay on the selected API host');
  return url;
}
...
function buildUpstreamRequest(input = {}) {
  ...
  if (providerId && providerId !== 'custom') {
    ...
    path = validatePath(input.customPath);
