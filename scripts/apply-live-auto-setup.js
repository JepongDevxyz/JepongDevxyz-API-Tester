...
function ensure(search, replacement, label) {
...
const probeFunction = [
  "async function probeProviderSetup(){",
  ...
  "  $('prompt').value='Reply only with OK';$('maxTokens').value='8';",
  "  try{",
  ...
  "        if(probeModel){state.models=normalizeModels(ids);state.selected=state.models.find(model=>model.id===probeModel)||{id:probeModel,label:probeModel};$('selectedModelName').textContent=state.selected.label||probeModel;$('selectedModelId').textContent=probeModel;renderModelList(state.models)}",
  "      }",
  "    }",
  "    if(!probeModel)probeModel=state.selected?.id||'';",
  "    if(!probeModel){setStatus('Auto setup could not discover a usable model. Enter a Manual model ID, then press Auto setup again.','bad');setResult(JSON.stringify({provider:provider.name,workingSetup:null,attempts},null,2));return}",
  "    for(let i=0;i<candidates.length;i++){",
  "      const endpoint=candidates[i];",
  "      $('endpointSelect').value=endpoint.id;applyPreset();",
  ...
  "  }finally{",
  "    $('prompt').value=original.prompt;$('maxTokens').value=original.maxTokens;button.disabled=false;$('loadBtn').disabled=false;$('testActiveBtn').disabled=false;$('testAllBtn').disabled=false;",
  "    if(!working){$('endpointSelect').value=original.endpoint;applyPreset();$('manualModel').value=original.manual;state.models=original.models;state.selected=original.selected;$('selectedModelName').textContent=original.selected?.label||'Load models first';$('selectedModelId').textContent=original.selected?.id||'No model selected';renderModelList(state.models)}",
  "  }",
  "}"
].join('\n');

const start = html.indexOf('async function probeProviderSetup(){');
if (start >= 0) {
  const end = html.indexOf('\nfunction syncKeys()', start);
  if (end < 0) throw new Error('Could not locate probe function end');
  html = html.slice(0, start) + probeFunction + html.slice(end);
} else {
  if (!html.includes(requestKeyLine)) throw new Error('requestKey marker not found');
  html = html.replace(requestKeyLine, requestKeyLine + '\n' + probeFunction);
}

html = html.replace("$('autoSetupBtn').onclick=applyPreset;", "$('autoSetupBtn').onclick=probeProviderSetup;");
if (!html.includes("$('autoSetupBtn').onclick=probeProviderSetup;")) throw new Error('Auto setup button binding missing');

fs.writeFileSync(file, html);
console.log('Live auto-setup browser function repaired.');
